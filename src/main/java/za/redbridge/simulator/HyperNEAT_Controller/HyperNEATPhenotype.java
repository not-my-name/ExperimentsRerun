...
public class HyperNEATPhenotype implements Phenotype {
    ...
    public HyperNEATPhenotype(NEATNetwork network, Morphology morphology, int[] sensorBitMask) { //need to change the SensorMorpholgy to just Morphology that Daniel made
    ...
    public Double2D step(List<List<Double>> sensorReadings) {
        ...
        for (int i = 0, n = input.size(); i < n; i++) {
			...
			if(sensorBitMask[i] == 0) { //check if the current sensor is broken
				input.setData(i, 0); //if so, dont take any input
			}
			else {
