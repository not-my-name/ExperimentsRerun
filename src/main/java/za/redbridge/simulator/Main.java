...
public class Main {
	...
	private static int[] sensorMorphologies = new int[]{2,5,6}; //the sensor morphologies that need to be iterated over
	...
	public static void main(String args[]) throws IOException, ParseException{
		...
		for(int j = 0; j < 3; j++) {
			...
			for(int k = 0; k < 4; k++) { //iterating over the different complexity levels
				...
				String folderDir = "/HyperNEATExperiments/Hybrid/Morphology_"
	...
	private static class Args {
        ...
        private int numGenerations = 2;//100;

        @Parameter(names = "-p", description = "Initial population size")
        private int populationSize = 2;//150;

        @Parameter(names = "--sim-runs", description = "Number of simulation runs per iteration")
        private int simulationRuns = 2;//5;
